      ...
      try {
        ...
        if (res.ok) {
          ...
          if (orders && orders.length > 0) {
            ...
            const coverMedia = order.cover_media_url || 'https://lashmenu.com/modelos/mosaico/assets/img/Hero.png';
...
<body>
  ...
  <p>${heroPhrase}</p>
    ...
    // Se usuário comum acessar a raiz do subdomínio do cliente (ex: analaura.lashmenu.com), encaminha para o catálogo
    const url = new URL(request.url);
    if (url.pathname === '/' || url.pathname === '') {
      url.pathname = '/catalogo/index.html';
